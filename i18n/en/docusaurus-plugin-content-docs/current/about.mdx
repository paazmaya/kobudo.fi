...
The site is bilingual: the primary language is **Finnish**, with a full **English** translation available. The site URL is `kobudo.fi` (a Finnish-registered domain), and the default language for visitors without an expressed preference is Finnish.
...
Selected site images are generated locally with [ComfyUI](https://github.com/comfyanonymous/ComfyUI) using a custom ERNIE-image model workflow. Running generation locally keeps prompt iteration fast, makes outputs reproducible, and avoids third-party image API dependencies for visual assets.
